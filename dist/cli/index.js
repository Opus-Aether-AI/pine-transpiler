#!/usr/bin/env node
import { a as transpileToStandaloneFactory, i as transpileToPineJS, r as transpile, t as canTranspilePineScript, v as getMappingStats } from "../src-BHesQFJI.js";
...
function commandTranspile(file, options) {
	...
	if (format === "js") try {
		...
		const indicatorId = options.id || deriveIndicatorId(file);
		const indicatorName = options.name;
		const result = transpileToStandaloneFactory(code, indicatorId, indicatorName);
