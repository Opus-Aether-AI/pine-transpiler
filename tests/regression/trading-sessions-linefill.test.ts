...
const TRADING_SESSIONS_SOURCE = `//@version=6
...
describe('Trading Sessions linefill runtime', () => {
  it('runs the full script without throwing and emits drawing events including linefill.new', () => {
    const transpiled = transpileToPineJS(
      TRADING_SESSIONS_SOURCE,
      'trading_sessions_linefill',
      'Trading Sessions Linefill',
    );
    ...
    expect(events.some((event) => event.call === 'label.new')).toBe(true);
    expect(events.some((event) => event.call === 'linefill.new')).toBe(true);
  });

  it('reorders out-of-order named linefill.new arguments to (line1, line2, color)', () => {
    const transpiled = transpileToPineJS(
      [
        '//@version=6',
        'indicator("t", overlay=true)',
        'l1 = line.new(bar_index, high, bar_index, low)',
        'l2 = line.new(bar_index, high, bar_index, low)',
        'linefill.new(color = color.red, line1 = l1, line2 = l2)',
      ].join('\n'),
      'linefill_named_args',
      'Linefill Named Args',
    );
    expect(transpiled.success).toBe(true);
    expect(transpiled.indicatorFactory?.__pineJsBody).toContain(
