...
const DEFAULT_FIXTURE_GROUP = 'curated';
...
function canonicalizeValue(value: unknown): unknown {
  ...
  if (typeof value === 'object') {
    ...
    if (typeof handleId === 'number' && Number.isFinite(handleId)) {
...
function buildParitySignature(frames: HarnessBarFrame[]): string {
  const payload = frames.map((frame) => ({
    ...
    plots: canonicalizeValue(frame.plots),
