 ...
 * as JavaScript", etc.).
 */
export function isUnsafeEvalCspError(error: unknown): boolean {
 ...
 */
export function withCspEvalHint(error: unknown): string {
 ...
 */
export function appendCspHint(error: Error): Error {
  if (!isUnsafeEvalCspError(error)) return error;
  if (!error.message.includes(CSP_EVAL_HINT)) {
