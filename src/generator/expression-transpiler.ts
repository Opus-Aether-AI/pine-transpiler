 ...
 */
function transpileMathFunction(
  funcName: string,
  args: string[],
): string | null {
 ...
 */
function transpileTimeFunction(
  funcName: string,
  args: string[],
): string | null {
...
function transpileNAFunctions(expr: string): string {
  ...
  for (const rep of naReplacements.reverse()) {
    result =
      result.slice(0, rep.start) + rep.replacement + result.slice(rep.end);
  }
  ...
  let nzMatch: RegExpExecArray | null;
  const nzReplacements: Array<{
    start: number;
    end: number;
    replacement: string;
  }> = [];
...
function transpileHistoricalReferences(
  ...
  expression: string;
  historicalVars: Array<{ name: string; offset: number }>;
} {
...
function transpilePriceSources(expr: string): string {
  ...
  result = result.replace(
    /\btimeframe\.isintraday\b/g,
    'Std.isintraday(context)',
  );
  ...
  result = result.replace(
    /\btimeframe\.ismonthly\b/g,
    'Std.ismonthly(context)',
  );
  result = result.replace(
    /\btimeframe\.multiplier\b/g,
    'Std.interval(context)',
  );
  ...
  result = result.replace(
    /\bbarstate\.isrealtime\b/g,
    'Std.isrealtime(context)',
  );
  result = result.replace(/\bbarstate\.isnew\b/g, 'Std.isnew(context)');
  result = result.replace(
    /\bbarstate\.isconfirmed\b/g,
    'Std.isconfirmed(context)',
  );
 ...
 */
function transpileColorFunction(
  funcName: string,
  args: string[],
): string | null {
...
function transpileFunctionCalls(
  ...
  const functionPattern =
    /(\b(?:ta|math|timeframe|syminfo|barstate|str|array|color)\.\w+|\byear|\bmonth|\bweekofyear|\bdayofmonth|\bdayofweek|\bhour|\bminute|\bsecond)\s*\(([^)]*)\)/g;
  ...
  while ((match = functionPattern.exec(expr)) !== null) {
    ...
    if (funcName.startsWith('ta.')) {
      replacement = transpileTAFunction(
        funcName,
        args,
        inputIds,
        variableNames,
      ...
      funcName.startsWith('timeframe.') ||
      isTimeDateFunction(funcName)
    ) {
...
export function transpileVariables(
  ...
  for (const variable of variables) {
    ...
    const availableVars = variableNames.slice(
      0,
      variableNames.indexOf(variable.name),
    );
    const transpiled = transpileExpressionWithMeta(
      variable.expression,
      inputIds,
      availableVars,
    );
